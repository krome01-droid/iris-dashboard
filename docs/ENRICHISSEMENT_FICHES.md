...
`POST /wp-json/iris/v1/enrich-from-gmb` — accepte un payload JSON Apify, fait le matching par raf_numero ou (raison_sociale + code_postal) et écrit les metas :
...
### Dashboard IRIS — UI à ajouter
Dans `iris-dashboard/src/app/admin-iris/enrichissement/page.tsx` :
- Compteur "Fiches à enrichir" (`WHERE meta 'note' vide`)
- Upload fichier JSON résultat Apify
- Trigger via fetch → `/wp-json/iris/v1/enrich-from-gmb`
...
| **2** | +1j | Développer endpoint `/iris/v1/enrich-from-gmb` + UI IRIS |
...
1. **Vérifier visuellement 3-4 fiches draft** (http://autoecole-inris.com/wp-admin/edit.php?post_type=auto-ecole&post_status=draft)
