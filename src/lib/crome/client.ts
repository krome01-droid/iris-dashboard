 ...
 *
 * Ce fichier est le même dans les dashboards de LOU, ANGÈLE, STAN et IRIS.
 * Seul `AGENT_ID` change.
 *
 * Chez IRIS, seule la RÉDACTION est utilisée. `submitPost` est là parce que le
 * fichier est partagé, mais autoecole-inris.com n'a aucune page sociale active
 * (décision du 04/08/2026, vérifiée dans `postiz_integrations` le 30/08) : le
 * social de l'enseigne est porté par ANGÈLE depuis inris-formations.com.
 */
...
// qui, ensuite, le dépose dans Webflow avec les identifiants de l'agent. Le
// hub n'a aucun accès à autoecole-inris.com, et c'est volontaire.
