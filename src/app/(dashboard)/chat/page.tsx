...
export default function ChatPage() {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      const res = await fetch("/admin-iris/api/conversations")
      if (res.ok) {
  ...
  function exportAsMarkdown() {
    ...
    const md = messages
      .map((m) => {
        const role = m.role === "user" ? "## Vous" : "## IRIS"
    ...
    a.href = url
    a.download = `conversation-iris-${new Date().toISOString().split("T")[0]}.md`
    a.click()
  ...
  function copyConversation() {
    if (messages.length === 0) return
    const text = messages
      .map((m) => `${m.role === "user" ? "Vous" : "IRIS"}: ${m.content}`)
      .join("\n\n")
