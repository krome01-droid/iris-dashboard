...
export default function SettingsPage() {
  ...
  async function testConnections() {
    setTesting(true)
    try {
      const res = await fetch("/admin-iris/api/settings/test-connections")
  ...
  async function runMigration() {
    setMigrating(true)
    try {
      const res = await fetch("/admin-iris/api/db/migrate", { method: "POST" })
      const data = await res.json()
  ...
  async function checkGoogleStatus() {
    try {
      const res = await fetch("/admin-iris/api/google")
  ...
  async function connectGoogle() {
    ...
    try {
      const res = await fetch("/admin-iris/api/google?action=authorize")
      if (res.ok) {
        const data = await res.json()
        if (data.authUrl) {
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Créer les 5 tables IRIS dans la base MySQL WordPress (wp_iris_content_log,
              wp_iris_social_posts, wp_iris_seo_reports, wp_iris_editorial_calendar,
              wp_iris_conversations).
            </p>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm text-muted-foreground">
            <p>
              <strong>Agent :</strong> IRIS v1.0 — Communication Manager
            </p>
            <p>
              <strong>Site :</strong> autoecole-inris.com
            </p>
            <p>
