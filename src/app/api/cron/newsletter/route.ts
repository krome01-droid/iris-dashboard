  ...
  try {
    ...
    const response = await client.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `Tu es IRIS, redacteur de la newsletter hebdomadaire d'autoecole-inris.com.
      ...
      <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif;">
        ...
        <div style="padding: 30px 40px;">
          ...
          <table width="100%" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td align="center">
                <a href="https://autoecole-inris.com" style="display: inline-block; background-color: #e31e44; color: #ffffff; font-family: Arial, sans-serif; font-size: 16px; font-weight: bold; text-decoration: none; padding: 14px 36px; border-radius: 8px;">${newsletter.cta_text}</a>
              </td>
            </tr>
          </table>
        </div>
        ...
        <div style="padding: 20px 40px; background-color: #1a1c1c; text-align: center;">
          <p style="font-size: 12px; color: #999999; margin: 0; font-family: Arial, sans-serif;">
            Auto-École INRI'S — Le comparateur d'auto-ecoles en France
          </p>
        </div>
      </div>`
    ...
    try {
      await execute(
        `INSERT INTO wp_iris_content_log (title, type, status, content_markdown, meta_json, created_by)
         VALUES (?, 'newsletter', 'published', ?, ?, 'iris-newsletter')`,
        [
          newsletter.subject,
          newsletter.edito,
          JSON.stringify({
