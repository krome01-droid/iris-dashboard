  ...
  try {
    ...
    try {
      recentActivity = await query<{
        ...
        "SELECT title, type, status, created_at FROM wp_iris_content_log ORDER BY created_at DESC LIMIT 5",
      )
    } catch {
    ...
    try {
      upcomingEvents = await query<{
        ...
        "SELECT title, content_type, planned_date FROM wp_iris_editorial_calendar WHERE planned_date >= CURDATE() ORDER BY planned_date ASC LIMIT 5",
      )
    } catch {
      // DB may not be migrated yet
    }
    ...
    try {
      const seoRows = await query<{ data_json: string }>(
        "SELECT data_json FROM wp_iris_seo_reports ORDER BY created_at DESC LIMIT 1",
      )
      if (seoRows.length > 0) {
