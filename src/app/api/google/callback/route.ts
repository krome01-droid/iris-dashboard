...
export async function GET(req: NextRequest) {
  ...
  try {
    const baseUrl = process.env.NEXTAUTH_URL || "https://agent.autoecole-inris.com/admin-iris"
    ...
    try {
      await execute(
        `INSERT INTO wp_iris_settings (\`key\`, \`value\`, updated_at)
    ...
    return new Response(
      htmlPage(
        ...
        `<p>Le refresh token a été obtenu et sauvegardé en base de données.</p>
         ...
         <p style="margin-top:16px">Si vous devez l'ajouter manuellement dans Vercel, récupérez-le via la table <code>wp_iris_settings</code> (clé <code>GOOGLE_REFRESH_TOKEN</code>).</p>
         <p style="margin-top:16px"><a href="/admin-iris/settings" style="color:#e31e44;">Retour aux paramètres</a></p>`,
        false,
      ),
      { headers: { "Content-Type": "text/html; charset=utf-8" } },
    )
...
<title>${title} — IRIS Dashboard</title>
<style>
